...
public class BookController {
    ...
    public ResponseEntity<PageResponse<BookResponse>> getAllBook(
        ...
        return ResponseEntity.ok(bookService.getAllBook(page,size,connectedUser));
    }
    ...
    public ResponseEntity<PageResponse<BookResponse>> getAllBookByOwner(
        ...
        return ResponseEntity.ok(bookService.getAllBookByOwner(page,size,connectedUser));
    }

    @GetMapping("/borrowed")
    public ResponseEntity<PageResponse<BorrowedBookResponse>> getAllBorrowedBook(
            @RequestParam(name = "page", defaultValue = "0", required = false)int page,
            @RequestParam(name = "size", defaultValue = "10", required = false)int size,
            Authentication connectedUser
    ){
        return ResponseEntity.ok(bookService.getAllBorrowedBook(page,size,connectedUser));
    }

    @GetMapping("/returned")
    public ResponseEntity<PageResponse<ReturnedBookResponse>> getAllReturnedBook(
            @RequestParam(name = "page", defaultValue = "0", required = false)int page,
            @RequestParam(name = "size", defaultValue = "10", required = false)int size,
            Authentication connectedUser
    ){
        return ResponseEntity.ok(bookService.getAllReturnedBook(page,size,connectedUser));
    }
